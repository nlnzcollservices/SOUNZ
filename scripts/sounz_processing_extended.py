...
def bib_updating(value, mms): 
    """Makes record in alma and extracts mms id
    Parameters:
        value(str) - bib record in xml format
        mms (str) - Alma mms id for record to update
    Returns:
        flag(bool) - True if something got wrong
        mms (str) - Alma mms id of updated record(  the same as imput )
    ...
    parameters = {"apikey": api_key}
    r = requests.put( url, headers=headers, data = value.replace("\\", "").replace('<datafield ind1=" " ind2="4" tag="264"><subfield code="c">copyrightcopyright</subfield></datafield>',""), params = parameters, verify= False)
    bib_grab = BeautifulSoup( r.text, 'lxml-xml' )
    try:
        mms = bib_grab.find( 'bib' ).find( 'mms_id' ).string  #looking for mms-id
        statement = 'MMS_ID: '+mms + "\n" +r.text +"\n"
...
###########################################################For updateing po_lines from text report###################################
            ...
            get_po_line(pol_no_atl)
            pol_no_atl, flag_po_atl = update_po_line( pol_no_atl, po_data )
            if not flag_po_atl:
                 print(pol_no_atl, " - updated")
            else:
                 print(pol_no_atl, " - failed to update")     
...
##########################################SETTINGS##############################################################    
text_file_path = os.path.join(working_folder,"log","reports","sounz_phys_atl_{}.txt".format(Times.underscore())) 
